 ...
 */
export function getPAcoefficient_IOM2005(gender: GenderInternal, activityLevel: ActivityLevel): number {
  if (gender === 'male') {
    switch (activityLevel) {
      case 'sedentary': return 1.0;
      case 'low_active': return 1.11;
      case 'active': return 1.25;
      case 'very_active': return 1.48;
      default: return 1.0;
    }
  } else { // female
    switch (activityLevel) {
      case 'sedentary': return 1.0;
      case 'low_active': return 1.12;
      case 'active': return 1.27;
      case 'very_active': return 1.45;
      default: return 1.0;
    }
  }
}
...
export function calculateGET_IOM2005(params: GETCalculationParameters): number {
  const { age, gender, weightKg, heightCm, activityLevel, isObeseChildOrAdolescent, isPregnant, pregnancyTrimester, isLactating } = params;
  const heightM = heightCm / 100;
  ...
  if (age >= 19) { // Adults
    if (gender === 'male') {
      ...
      eer = 662 - (9.53 * age) + paCoefficient * ((15.91 * weightKg) + (539.6 * heightM));
    } else { // female adult
      // Adult women: GET = 354 - (6.91 × Age) + PA × [(9.36 × Weight) + (726 × Height)]
      eer = 354 - (6.91 * age) + paCoefficient * ((9.36 * weightKg) + (726 * heightM));
      ...
      if (isLactating) {
        // This needs more nuance (e.g., first 6 months vs. after, exclusivity of breastfeeding)
        // Simplified version from some DRI summaries:
        // Typically +500 kcal for first 6 months, +400 kcal after, but this can vary.
        // Using a general value from one of the provided sources for pregnancy/lactation.
        // The prompt links to a PDF that might have more specific values for lactation.
        // For now, using a common figure, assuming 0-6 months postpartum:
        eer += 330; // Example adjustment for lactation (0-6 months postpartum)
        // Or if >6 months postpartum: eer += 400;
      }
    }
  } else if (age >= 3 && age <= 18) { // Children and Adolescents
    if (gender === 'male') {
      if (isObeseChildOrAdolescent) {
        // Obese boys (3-18 years): GET = 114 - (50.9 × Age) + PA × [(19.5 × Weight) + (1161.4 × Height)]
        eer = 114 - (50.9 * age) + paCoefficient * ((19.5 * weightKg) + (1161.4 * heightM));
      } else {
        // Boys (3-18 years): GET = 88.5 - (61.9 × Age) + PA × [(26.7 × Weight) + (903 × Height)] + 25
        eer = 88.5 - (61.9 * age) + paCoefficient * ((26.7 * weightKg) + (903 * heightM)) + 25;
      }
    } else { // female child/adolescent
      if (isObeseChildOrAdolescent) {
        // Obese girls (3-18 years): GET = 389 - (41.2 × Age) + PA × [(15.0 × Weight) + (701.6 × Height)]
        eer = 389 - (41.2 * age) + paCoefficient * ((15.0 * weightKg) + (701.6 * heightM));
      } else {
        ...
        eer = 135.3 - (30.8 * age) + paCoefficient * ((10.0 * weightKg) + (934 * heightM)) + 20;
      }
    }
  }
  // Add a clause for age < 3 if formulas are available and needed.
...
export function calculateMacronutrientRanges(
  ...
  percentages: MacronutrientPercentageRanges = {
    ...
    proteins: { min: 0.10, max: 0.35 },    
    lipids: { min: 0.20, max: 0.35 },      
  }
...
export interface MicronutrientInput extends Omit<BasicPatientInfo, 'heightCm' | 'weightKg'> { 
  isPregnant?: boolean;
  isLactating?: boolean;
...
export function getRiboflavinRecommendation(params: MicronutrientInput): number {
  const { gender } = params;
...
export function getPyridoxineRecommendation(params: MicronutrientInput): number {
  const { age, gender } = params;
