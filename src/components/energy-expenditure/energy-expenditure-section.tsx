...
export function EnergyExpenditureSection({ patient }: EnergyExpenditureSectionProps) {
  ...
  const renderFormField = (item: any, field: any, index: number, isTextarea: boolean = false) => {
    ...
    if (item.component === Input) {
      ...
      controlElement = (
        <Select onValueChange={field.onChange} defaultValue={field.value as string | undefined}>
          ...
          <SelectContent>{item.options?.map((opt: any) => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent>
        </Select>
      );
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
              <Card>
                ...
                <CardContent className="space-y-0">
                  {generalDataFields.map((item, index) => (
                     <FormField key={item.name} control={form.control} name={item.name} render={({ field }) => renderFormField(item, field, index, item.component === Textarea)} />
                  ))}
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {physicalActivityFields.map((field, index) => (
                    <Card key={field.id} className={`p-4 space-y-3 relative shadow-sm border ${index % 2 === 0 ? "bg-muted/50" : "bg-transparent"}`}>
                      <Button type="button" variant="ghost" size="icon" className="absolute top-2 right-2 text-destructive hover:text-destructive h-6 w-6" onClick={() => removePhysicalActivity(index)}>
                        <Trash2 className="h-4 w-4" /><span className="sr-only">Remover Atividade</span>
                      </Button>
                      <FormField control={form.control} name={`physicalActivities.${index}.type`} render={({ field: actField }) => (
                        <FormItem className="flex flex-col sm:flex-row sm:items-center sm:gap-4">
                            <FormLabel className="sm:w-1/3 mb-1 sm:mb-0 sm:text-right">Tipo de Atividade</FormLabel>
                            <div className="sm:w-2/3">
                                <Select
                                    onValueChange={(value) => {
                                    actField.onChange(value);
                                    const selectedMet = activityMetsValues[value];
                                    form.setValue(`physicalActivities.${index}.mets`, selectedMet ?? undefined);
                                    }}
                                    defaultValue={actField.value}
                                >
                                    <FormControl><SelectTrigger><SelectValue placeholder="Selecione uma atividade" /></SelectTrigger></FormControl>
                                    <SelectContent>{activityOptions.map(opt => (<SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>))}</SelectContent>
                                </Select>
                                <FormMessage className="mt-1 text-xs"/>
                            </div>
                        </FormItem>
                        )} />
                      <FormField control={form.control} name={`physicalActivities.${index}.duration`} render={({ field: actField }) => (<FormItem className="flex flex-col sm:flex-row sm:items-center sm:gap-4"><FormLabel className="sm:w-1/3 mb-1 sm:mb-0 sm:text-right">Duração</FormLabel><div className="sm:w-2/3"><FormControl><Input placeholder="Ex: 30 min/dia" {...actField} /></FormControl><FormDescription className="text-xs mt-1">min/dia ou vezes/semana</FormDescription><FormMessage className="mt-1 text-xs"/></div></FormItem>)} />
                      <FormField control={form.control} name={`physicalActivities.${index}.mets`} render={({ field: actField }) => (<FormItem className="flex flex-col sm:flex-row sm:items-center sm:gap-4"><FormLabel className="sm:w-1/3 mb-1 sm:mb-0 sm:text-right">METs (Opcional)</FormLabel><div className="sm:w-2/3"><FormControl><Input type="number" step="0.1" placeholder="Ex: 3.5" {...actField} value={actField.value ?? ""} /></FormControl><FormMessage className="mt-1 text-xs"/></div></FormItem>)} />
                      <FormField control={form.control} name={`physicalActivities.${index}.intensity`} render={({ field: actField }) => (<FormItem className="flex flex-col sm:flex-row sm:items-center sm:gap-4"><FormLabel className="sm:w-1/3 mb-1 sm:mb-0 sm:text-right">Intensidade Percebida</FormLabel><div className="sm:w-2/3"><Select onValueChange={actField.onChange} defaultValue={actField.value}><FormControl><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger></FormControl><SelectContent>{activityIntensityOptions.map(opt => (<SelectItem key={opt} value={opt}>{opt}</SelectItem>))}</SelectContent></Select><FormMessage className="mt-1 text-xs"/></div></FormItem>)} />
                    </Card>
                  ))}
              ...
              <Card>
                  ...
                  <CardContent className="space-y-0">
                      <FormField control={form.control} name="workActivity.description" render={({ field }) => renderFormField({label: "Descrição da Atividade Principal", component: Input, placeholder: "Ex: Trabalho de escritório, Construção"}, field, 0)} />
                      <FormField control={form.control} name="workActivity.timeSpent" render={({ field }) => renderFormField({label: "Tempo Gasto", component: Input, placeholder: "Ex: 8 horas/dia"}, field, 1)} />
                      <FormField control={form.control} name="workActivity.mets" render={({ field }) => renderFormField({label: "METs (Opcional)", component: Input, type:"number", step:"0.1", placeholder: "Ex: 1.5"}, field, 2)} />
                      <FormField control={form.control} name="workActivity.occupationalActivityFactor" render={({ field }) => renderFormField({label: "Fator Atividade Ocupacional", component: Input, placeholder: "Ex: Leve, Moderada, 1.2", description: "Ou use METs."}, field, 3)} />
                  </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {otherActivityFields.map((field, index) => (
                     <Card key={field.id} className={`p-4 space-y-3 relative shadow-sm border ${index % 2 === 0 ? "bg-muted/50" : "bg-transparent"}`}>
                      <Button type="button" variant="ghost" size="icon" className="absolute top-2 right-2 text-destructive hover:text-destructive h-6 w-6" onClick={() => removeOtherActivity(index)}>
                        <Trash2 className="h-4 w-4" /><span className="sr-only">Remover Atividade</span>
                      </Button>
                      <FormField control={form.control} name={`otherActivities.${index}.type`} render={({ field: actField }) => (<FormItem className="flex flex-col sm:flex-row sm:items-center sm:gap-4"><FormLabel className="sm:w-1/3 mb-1 sm:mb-0 sm:text-right">Tipo de Atividade</FormLabel><div className="sm:w-2/3"><FormControl><Input placeholder="Ex: Tarefas domésticas, Estudo" {...actField} /></FormControl><FormMessage className="mt-1 text-xs"/></div></FormItem>)} />
                      <FormField control={form.control} name={`otherActivities.${index}.duration`} render={({ field: actField }) => (<FormItem className="flex flex-col sm:flex-row sm:items-center sm:gap-4"><FormLabel className="sm:w-1/3 mb-1 sm:mb-0 sm:text-right">Duração</FormLabel><div className="sm:w-2/3"><FormControl><Input placeholder="Ex: 1 hora/dia" {...actField} /></FormControl><FormMessage className="mt-1 text-xs"/></div></FormItem>)} />
                      <FormField control={form.control} name={`otherActivities.${index}.mets`} render={({ field: actField }) => (<FormItem className="flex flex-col sm:flex-row sm:items-center sm:gap-4"><FormLabel className="sm:w-1/3 mb-1 sm:mb-0 sm:text-right">METs (Opcional)</FormLabel><div className="sm:w-2/3"><FormControl><Input type="number" step="0.1" placeholder="Ex: 2.0" {...actField} value={actField.value ?? ""} /></FormControl><FormMessage className="mt-1 text-xs"/></div></FormItem>)} />
                    </Card>
                  ))}
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {patient.energyExpenditureRecords.map((record, tblIndex) => (
                    <TableRow key={record.id} className={tblIndex % 2 === 0 ? "bg-muted/50" : "bg-transparent"}>
                      ...
                      <TableCell className="max-w-[200px] truncate">{record.workActivity?.description || "N/A"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
