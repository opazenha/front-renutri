...
import {
  ...
  TrendingDown,
  TrendingUp,
} from "lucide-react"; // Renamed Circle to CircleIcon
import {
  Cell,
  Legend,
  Pie,
...
export function PatientSummarySidebar({
  ...
  totalPlannedFat,
  totalPlannedFiber,
}: PatientSummarySidebarProps) {
  const totalPlannedMacrosKcal =
    totalPlannedProtein * 4 + totalPlannedCarbs * 4 + totalPlannedFat * 9;
  ...
  const macroData = [
    {
      ...
      percentage:
        totalPlannedMacrosKcal > 0
          ? ((totalPlannedProtein * 4) / totalPlannedMacrosKcal) * 100
          : 0,
      fill: COLORS.protein,
    },
    {
      ...
      percentage:
        totalPlannedMacrosKcal > 0
          ? ((totalPlannedCarbs * 4) / totalPlannedMacrosKcal) * 100
          : 0,
      fill: COLORS.carbs,
    },
    {
      ...
      percentage:
        totalPlannedMacrosKcal > 0
          ? ((totalPlannedFat * 9) / totalPlannedMacrosKcal) * 100
          : 0,
      fill: COLORS.fat,
    },
  ];
  ...
  const energyProgress =
    targetEnergy > 0 ? (totalPlannedEnergy / targetEnergy) * 100 : 0;

  return (
    <Card className="h-full flex flex-col shadow-lg">
      <CardHeader className="pb-3">
        <CardTitle className="text-lg text-primary">
          Resumo para {patient.name}
        </CardTitle>
        <CardDescription className="text-xs">
          Comparativo do plano atual com as metas.
        </CardDescription>
      </CardHeader>
          ...
          <CardContent className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {totalPlannedEnergy.toFixed(0)}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Meta:</span>{" "}
              <span className="font-semibold">{targetEnergy.toFixed(0)}</span>
            </div>
            <Progress
              value={energyProgress > 100 ? 100 : energyProgress}
              className="h-2"
            />
            <div
            ...
            >
              ...
              <span className="font-semibold flex items-center">
                {energyDifference > 0 ? (
                  ...
                  <CircleIcon className="h-3 w-3 mr-1 fill-muted-foreground" />
                )}
        ...
        <Card>
          <CardHeader className="pb-2 pt-4">
            <CardTitle className="text-md flex items-center">
              ...
              Distribuição de Macronutrientes (Planejado)
            </CardTitle>
          </CardHeader>
                ...
                <PieChart margin={{ top: 5, right: 5, bottom: 25, left: 5 }}>
                  ...
                  <Pie
                    ...
                    label={({ name, percentage }) =>
                      `${name.substring(0, 4)}. (${percentage.toFixed(0)}%)`
                    }
                  ...
                  <Tooltip
                    formatter={(value: number, name, props) => [
                      `${props.payload.percentage.toFixed(1)}% (${value.toFixed(
                        0
                      )} kcal)`,
                      name,
                    ]}
        ...
        <Card>
          <CardHeader className="pb-2 pt-4">
            <CardTitle className="text-md">
              Metas de Macronutrientes (g)
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            <div className="flex justify-between">
              <span>Proteínas (Meta):</span>{" "}
              <span className="font-semibold">
                {targetProteinGrams.toFixed(1)}g
              </span>
            </div>
            <div className="flex justify-between">
              <span>Carboidratos (Meta):</span>{" "}
              <span className="font-semibold">
                {targetCarbsGrams.toFixed(1)}g
              </span>
            </div>
            <div className="flex justify-between">
              <span>Gorduras (Meta):</span>{" "}
              <span className="font-semibold">
                {targetFatGrams.toFixed(1)}g
              </span>
            </div>
            <Separator className="my-2" />
            <div className="flex justify-between">
              <span>Fibras (Planejado):</span>{" "}
              <span className="font-semibold">
                {totalPlannedFiber.toFixed(1)}g
              </span>
            </div>
          </CardContent>
        </Card>
